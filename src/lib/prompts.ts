...
## 임무: 사용자 관점에서 창의적 개선안 제시

이 서비스를 실제 사용자 입장에서 써 보고, 차별화된 개선 아이디어를 제시하세요.

### 분석 관점
- 경쟁 서비스에는 있지만 여기에 없는 기능
- 사용자가 "이런 것도 되면 좋겠다"고 느낄 순간
- 2026년 트렌드를 적용해 UX를 확 바꿀 수 있는 포인트

### 출력 형식
1. **[개선 아이디어]**: [사용자 시나리오] → [구현 난이도 ⭐~⭐⭐⭐]
2. **[개선 아이디어]**: [사용자 시나리오] → [구현 난이도 ⭐~⭐⭐⭐]

### 💡 Quick Win (최소 노력 최대 효과)
[구체적 1가지]
...
## 임무: 실제 사용자로서 UI/UX 불편점 토론

이 서비스를 매일 사용하는 사용자 관점에서 프론트엔드 경험을 평가하세요.

### 분석 관점
- 화면 전환 시 로딩/깜빡임
- 폼 입력의 유효성 피드백
- 모바일에서의 터치 타겟 크기
- 에러 발생 시 사용자에게 보이는 화면
- 자주 쓰는 기능까지의 클릭 수

### 출력 형식
1. **[불편 상황]**: [구체적 시나리오] → [개선안]
2. **[불편 상황]**: [구체적 시나리오] → [개선안]
...
function getModeratorPrompt(stage: DebateStageId, roleList: string, command?: DebateCommand, rules?: string): string {
  // user_perspective 단계: 사용자 관점 토론 종합
  if (stage === "user_perspective") {
    return `당신은 Google PM 출신의 프로젝트 리더입니다. 사용자 관점 토론을 종합하여 실행 가능한 UX 개선 계획을 정리합니다.
...
### 2. UX 개선 우선순위
| 순위 | 개선 항목 | 영향도 | 구현 난이도 | 담당 |
(가장 임팩트 큰 순으로 3-5개)

### 3. 즉시 반영할 사항
- (모든 전문가 동의, 바로 구현 가능한 항목)

### 4. 추후 검토 사항
- (의견 갈림 또는 추가 데이터 필요 항목)

### 5. 사용자 시나리오 최종 정리
[가장 중요한 사용 시나리오 1개를 단계별로 서술]
...
## 질문 규칙
- 한국어로 질문
- 핵심 질문 최대 3개 (객관식 2개, 주관식 1개)
- 답이 없어도 합리적 기본값으로 진행 가능한 내용은 묻지 말고 선택지에 "추천 기본값"을 포함
...
## 질문 규칙
- 한국어로 질문
- ${role.koreanName} 관점에서 계획을 크게 바꿀 질문만 최대 2개
- 객관식 1개와 주관식 1개 이내로 제한
- 답이 없어도 합리적 기본값으로 진행 가능한 내용은 묻지 말고 선택지에 "추천 기본값"을 포함
...
## 질문 규칙
- 한국어로 질문
- ${role.koreanName} 관점에서 구현 방향을 크게 바꿀 질문만 최대 2개
- 객관식 1개와 주관식 1개 이내로 제한
- 답이 없어도 합리적 기본값으로 진행 가능한 내용은 묻지 말고 선택지에 "추천 기본값"을 포함
...
## 질문 규칙
- 한국어로 질문
- ${role.koreanName} 관점에서 개발 방향을 크게 바꿀 질문만 최대 3개
- **객관식과 주관식을 섞어서 질문** (객관식 1-2개, 주관식 1개)
- 답이 없어도 기본값으로 진행 가능한 내용은 묻지 말고 "추천 기본값" 선택지를 포함
- 객관식: 선택지 3-5개를 제시 (가장 흔한 옵션들로 구성)
- 주관식: 구체적인 요구사항이나 상세 설명이 필요한 질문
- 각 질문에 왜 이 정보가 필요한지 괄호로 간단히 설명
- 질문 번호를 매겨주세요
...
아래에 자동 계획 하네스의 산출물과 이전 PRD, 사용자 피드백이 주어집니다.

## 개선 규칙
- 한국어로 작성
- 이전 PRD의 구조와 내용을 최대한 유지하면서, 사용자 피드백을 우선 반영하라
- 하네스 산출물(RequirementSpec, CPS, GeneratedPlan, Evaluation, GitHub Research)과 충돌하는 수정은 하지 마라
...
5. GitHub Research — 참고 가능한 오픈소스 패턴과 주의점
...
- GitHub Research가 있으면 "참고 레퍼런스와 차용 패턴" 섹션에 저장소 URL, 차용할 패턴, 주의점을 요약하라
- 참고 저장소의 코드를 그대로 복제하지 말고, 제품에 맞는 패턴만 반영하라
...
export function buildHarnessPrdUserMessage(
  artifacts: { requirementSpec?: any; cps?: any; generatedPlan?: any; evaluation?: any; githubResearch?: any },
): string {
  const parts: string[] = [];

  if (artifacts.githubResearch) {
    parts.push("## GitHub Research");
    parts.push(JSON.stringify(artifacts.githubResearch, null, 2));
  }
...
export function buildHarnessUiUserMessage(
  artifacts: { requirementSpec?: any; cps?: any; generatedPlan?: any; githubResearch?: any },
): string {
  const parts: string[] = [];
