...
export async function runPlanHarness(
  ...
  const artifacts: PlanHarnessArtifacts = {
    githubResearch: input.githubResearch,
    lintIssues: [],
    attempts: [],
  };
  ...
  if (input.githubResearch) {
    emit({ event: "github_research", research: input.githubResearch, timestamp: Date.now() });
  }
  ...
  const cpsResult = await runWithRetry(artifacts, "cps", emit, signal, genModel,
    async (repairCtx) => {
      const { system, user } = buildCpsPrompt(artifacts.requirementSpec!, input.githubResearch);
      const raw = await callStructured(system, repairCtx ? `${user}\n\n${repairCtx}` : user, 4096, signal, genModel);
    ...
    const generateResult = await runWithRetry(artifacts, "generate", emit, signal, genModel,
      async (retryCtx) => {
        const { system, user } = buildPlanPrompt(
          artifacts.requirementSpec!,
          artifacts.cps!,
          retryCtx || repairContext,
          input.githubResearch,
        );
...
function emitCompleted(emit: OnEvent, success: boolean, artifacts: PlanHarnessArtifacts) {
  const parts: string[] = [];
  if (artifacts.githubResearch?.references.length) {
    parts.push(`GitHub 레퍼런스 ${artifacts.githubResearch.references.length}개 검토`);
  }
  ...
  emit({
    event: "completed",
    success,
    githubResearch: artifacts.githubResearch,
...
- missingInfo: 입력에서 빠진 핵심 결정사항 중 사용자 판단 없이는 위험한 것만 나열. GitHub Research Brief나 일반적인 제품 기본값으로 합리적으로 정할 수 있으면 묻지 말고 assumptions에 "가정:"으로 기록.
...
반환 스키마:
{
  ...
  const research = formatGitHubResearch(input.githubResearch);
  if (research) {
    user += `\n\n## GitHub Research Brief\n${research}`;
  }
...
function buildCpsPrompt(requirementSpec: RequirementSpec, githubResearch?: GitHubResearchBrief) {
...
반환 스키마:
{
  ...
  const research = formatGitHubResearch(githubResearch);
  if (research) {
    user += `\n\n## GitHub Research Brief\n${research}`;
  }
...
- GitHub Research Brief가 있으면 검증된 패턴은 task/detail/risks/acceptanceCriteria에 반영하되, 저장소를 그대로 복제하지 마라.
- archived, 보안 경고, hosted-only 기능 같은 주의점은 risks에 반영하라.
...
function formatGitHubResearch(research?: GitHubResearchBrief): string {
  if (!research) return "";

  const refs = research.references
    .slice(0, 5)
    .map((ref, i) => [
      `${i + 1}. ${ref.repository} (${ref.url})`,
      `   - stars: ${ref.stars}, language: ${ref.language || "unknown"}, archived: ${ref.archived ? "yes" : "no"}`,
      ref.description ? `   - description: ${ref.description}` : "",
      ref.patterns.length > 0 ? `   - patterns: ${ref.patterns.join(" | ")}` : "",
      ref.cautions.length > 0 ? `   - cautions: ${ref.cautions.join(" | ")}` : "",
    ].filter(Boolean).join("\n"))
    .join("\n");
  ...
  return [
    ...
    research.patterns.length > 0 ? `cross-repo patterns: ${research.patterns.join(" | ")}` : "",
    research.cautions.length > 0 ? `cross-repo cautions: ${research.cautions.join(" | ")}` : "",
